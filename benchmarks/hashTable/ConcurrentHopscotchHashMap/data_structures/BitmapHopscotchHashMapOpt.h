#ifndef __BITMAP_HOPSCOTCH_HASHMAPOPT__
#define __BITMAP_HOPSCOTCH_HASHMAPOPT__
...
template <typename	_tKey,
		    typename	_tData,
			 ...
			 typename	_tLock,
			 typename	_tMemory>
class BitmapHopscotchHashMapOpt {
private:
...
public:// Ctors ................................................................
	...
	BitmapHopscotchHashMapOpt(_u32 inCapacity, _u32 concurrencyLevel) 
	...
	inline_ _tData putIfAbsent(const _tKey key,  const _tData data) {
	  //if(containsKey(key)) return data;
	...
	typedef _tKey keyvaluePair;
	struct getKeyVal {
	  Bucket* A;
	getKeyVal(Bucket* AA) : A(AA) {}
	  keyvaluePair operator() (intT i) {
	    return A[i]._key;
	    //return make_pair(A[i]._key,A[i]._data);
	  }
	};

	_seq<keyvaluePair> entries() {
	  const _u32 num_elm( _bucketMask + _INSERT_RANGE );
	  bool *FL = newA(bool,num_elm);
	  parallel_for (intT i=0; i < num_elm; i++) 
	    FL[i] = (_table[i]._hash != _tHash::_EMPTY_HASH);
	  _seq<keyvaluePair> R = sequence::pack((keyvaluePair*)NULL,FL,(int)0,(int)num_elm,getKeyVal(_table));
	  free(FL);
	  cout<<R.n<<endl;
	  return R;
	}
