#ifndef KMEANSBB_HAMERLY_ACC_H
#define KMEANSBB_HAMERLY_ACC_H
 ...
 * of each of the k clusters (seeding) and uses the accelerated Hamerly's 
 * method for moving those centers toward a local optimum (iterations).
 */
template <class F, class DST, class DSTSQ, class NRM, class NRMSQ, class INPD>
int kmeansbbHamerlyAcc (F data[N][D], int num, int dim, int clusters, F error, 
...
#undef kmeans
#define kmeans(data, num, dim, clusters, error, irounds, kmbbrounds, lfactor, frac, rseed, distance, distancesqr, norm, normsqr, inprod, bfscr, score, steps) (kmeansbbHamerlyAcc(data, num, dim, clusters, error, irounds, kmbbrounds, lfactor, frac, rseed, distance, distancesqr, norm, normsqr, inprod, bfscr, score, steps))
