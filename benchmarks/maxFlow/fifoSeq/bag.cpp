...
template <typename T>
inline void Pennant<T>::split(Pennant<T>& a, Pennant<T>& b) {
  a.els = this->els;
  a.l = this->l->r;
  a.r = NULL;
  a.shallow = 1;
...
{
  uintT count = this->size;
  uintT k = 1;
  for (uintT i = 0; i < this->fill; i++) {
    if (this->bag[i] != NULL)
      count += k*BLK_SIZE;
    k = k * 2;
  }
...
template <typename T>
inline void
Bag<T>::insert_blk(T blk[], uintT size)
{
  intT i;
...
template <typename T>
inline void
Bag<T>::split(Bag<T>& bag, Pennant<T>& p) {
  assert(this->fill);
  Pennant<T>* hi = this->bag[fill - 1];
  p.l = hi->l;
  p.r = hi->r;
  p.els = hi->els;
  p.shallow = 1;
  bag.size = size;
  bag.bag = new Pennant<T>*[BAG_SIZE];
  bag.fill = 0;
  for (uintT i = 0; i < fill - 1; ++i) {
    bag.bag[i] = this->bag[i];
    if (this->bag[i]) bag.fill = i+1;
  }
  bag.filling = filling;
  bag.shallow = 1;
}
...
template <typename T>
intT
Bag<T>::split(Pennant<T>* *p, intT pos)
{
