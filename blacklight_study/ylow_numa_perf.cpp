...
void test_thread(size_t numthreads, size_t thrid, size_t memlen) {
  ...
  for (size_t p = 0; p < numthreads; p+=16) {
    if (thrid == p) {
      std::cout << "Processor " << p << ":" << std::endl;
      for (size_t i = 0;i < numthreads; i+=16) {
  ...
  if (thrid == 0) {
    ...
    std::cout << "P0 to PN, varying Word Size, Varying Prefetch...\n";
    if (numthreads > 1) {
      size_t t = numthreads - 1;
      for (size_t pf = 0; pf <= 128; pf+=8) {
      runtest<char>(memlen, 0, t, false, pf);
      runtest<char>(memlen, 0, t, true, pf);
      runtest<uint16_t>(memlen, 0, t, false, pf);
      runtest<uint16_t>(memlen, 0, t, true, pf);
      runtest<uint32_t>(memlen, 0, t, false, pf);
      runtest<uint32_t>(memlen, 0, t, true, pf);
      runtest<uint64_t>(memlen, 0, t, false, pf);
      runtest<uint64_t>(memlen, 0, t, true, pf);
      }
    }
