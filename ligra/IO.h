...
graph<vertex> readGraphFromFile(char* fname, bool isSymmetric) {
  ...
  if(!isSymmetric) {
    ...
    free(offsets);
#if defined(OPENMP)
    __gnu_parallel::sort(temp,temp+m,pairFirstCmp<intE>());
#else
...
wghGraph<vertex> readWghGraphFromFile(char* fname, bool isSymmetric) {
  ...
  /* parallel_for(intT i=0;i<m;i++) { edgesAndWeights[2*i] = edges[i]; */
  /*   edgesAndWeights[2*i+1] = weights[i]; } */
  parallel_for (uintT i=0; i < n; i++) {
    uintT o = offsets[i];
    uintT l = ((i == n-1) ? m : offsets[i+1])-offsets[i];
    v[i].setOutDegree(l);
...
#if defined(OPENMP)
    __gnu_parallel::sort(temp,temp+m,pairFirstCmp<intPair>());
#else
...
wghGraph<vertex> readWghGraphFromBinary(char* iFile, bool isSymmetric) {
  ...
  if(!isSymmetric) {
    ...
    parallel_for(uintT i=0;i<n;i++){
      ...
      V[i].setInDegree(l);
      V[i].setInNeighbors((intE*)(inEdgesAndWghs+2*o));
      //v[i].inNghWeights = (intE*) (inEdgesAndWghs+o+m);
    }
    free(tOffsets);
