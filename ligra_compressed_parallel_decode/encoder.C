...
//defined in bytecode.h
//typedef pair<uintE,uintE> intEPair; 
  ...
  */
  ...
  if (!isSymmetric) {
   ...
   /* From here on-out, we use temp, and tOffsets to guarantee
    ...
    tOffsets[0] = 0; tOffsets[n] = m; inEdges[0] = temp[0].second;
...
(char* fname, bool isSymmetric, char* outFile) {
  ...
  */
  ...
  if (!isSymmetric) {
    ...
    cout << "compressing out edges..."<<endl;
    // Compress the out-edges.
    char *nEdges = parallelCompressWeightedEdges(edges, offsets, n, m,Degrees);
    long totalSpace = sizes[2] = offsets[n];
    free(edges);
    cout<<"writing out edges..."<<endl;
    //write to binary file
   ...
   /* From here on-out, we use temp, and tOffsets to guarantee
    ...
    parallel_for(long i=0;i<n;i++) 
      Degrees[i] = tOffsets[i+1]-tOffsets[i];
    cout << "compressing in edges..."<<endl;
    char *ninEdges = parallelCompressWeightedEdges(inEdges, tOffsets, n, m,Degrees);
    long tTotalSpace[0];
    tTotalSpace[0] = tOffsets[n];
    free(inEdges);
    cout << "writing in edges..."<<endl;
    ...
    out.write((char*)tOffsets,sizeof(uintT)*(n+1)); //write offsets
    out.write((char*)Degrees,sizeof(uintE)*n); //write degrees
    out.write(ninEdges,tTotalSpace[0]); //write edges
    ...
    out.write((char*)offsets,sizeof(uintT)*(n+1)); //write offsets
    out.write((char*)Degrees,sizeof(uintE)*n); //write degrees
    out.write(nEdges,totalSpace); //write edges
    out.close();
