...
#define GTREE_BASE_CASE 5000
#define GTREE_SEQSORT_BASE 1000
#define GTREE_SUBPROB_POW 0.5
...
// FIX: do we want a copy of the vertices? 
// if so, should delete things later. not here though cause 
// we want to point at this list
//    v.del();
    return result;
  }
...
// A hack to get around Cilk shortcomings
  ...
  struct findBlock {
    ...
    int log_div;
    point minpt;
    findBlock(gTreeNode *t, int d) : tr(t) { 
      log_div = d;
      blocksize = (tr->size) / (double)(1 << log_div);
      minpt = (tr->center).offsetPoint(0,tr->size/2);
    }
    int operator() (vertex* p) {
      return ptFindBlock(minpt,blocksize,log_div,p->pt);
    }
  };
...
void buildRecursiveTree(seq<vertex*> S, int* offsets, int quadrants, gTreeNode *newNodes, gTreeNode* parent, int nodesToLeft, int height, int depth) {
  ...
  if (height == 1) {
    ...
    for (int i=0; i< (1<<center.dimension()); i++) {
      point newcenter = (parent->center).offsetPoint(i, parent->size/4.0);
      parent->children[i] = new(newNodes + i + nodesToLeft*(1<<center.dimension())) gTreeNode(newcenter,parent->size/2.0);
...
/* newNodes is the memory to use for allocated gTreeNodes.  this is
  ...
  gTreeNode(seq<vertex*> S, point cnt, double sz, gTreeNode* newNodes, int numNewNodes)
  {
    ...
    if (logdivs > 1 && count > GTREE_BASE_CASE) {
      ...
      intSort::iSort(S.S,offsets,count,quadrants,findBlock(this,logdivs));

      numNewNodes = (1<<center.dimension());
      for (int i=0; i<logdivs;i++) {
	numNewNodes = (numNewNodes << center.dimension()) + (1<<center.dimension());
      }
	...
	numNewNodes = max(GTREE_ALLOC_FACTOR*max(count/gMaxLeafSize, 1<<center.dimension()),1<<center.dimension());
	nodeMemory = newA(gTreeNode,numNewNodes);
	newNodes = nodeMemory;
      }
	...
	/* //computing offsets sequentially...seems worse in practice
	int j = 0;
	for (int q=0; q<quadrants; q++) {
	  offsets[q] = j;
	  while (j < S.sz && (findQuadrant(S[j]) == q)) j++;
	}
	*/
      }
	...
	point newcenter = center.offsetPoint(i, size/4.0);
       	children[i] = cilk_spawn newTree(A,newcenter,size/2.0,newNodes+usedNodes,(numNewNodes - (1<<center.dimension()))*l/count + 1);
	usedNodes += (numNewNodes - (1<<center.dimension()))*l/count + 1;
	//	children[i] = cilk_spawn newTree(A,newcenter,size/2.0,newNodes+i,1);
      }
      cilk_sync;
      for (int i=0 ; i < quadrants; i++) 
