...
namespace dataGen {
  ...
  public:
    bool operator()(payload const& A, payload const& B) const {
      return (A.key<B.key) ? 1 : ((A.key>B.key) ? 0 : (A.key2<B.key2));
    }
  };
  ...
  class payloadCmpxxx : public std::binary_function <payload, payload, bool> {
  public:
    bool operator()(payload const& A, payload const& B) const {
      return A.key<B.key;
    }
  ...
  payload* randPayload(int s, int e) { 
    ...
    for (int i = 0; i < n; i++) {
      A[i].key = hash<int>(2*i+s); // breaks with cilk_for
      A[i].key += ((long) hash<int>(2*i+1+s))<<32; // breaks with cilk_for
      A[i].key2 = hash<int>(2*i+1+s); // breaks with cilk_for
    }
    return A;
  }
